        ...
        <ul class="projects__list">
            <li v-for="post in posts" :key="post._id" class="projects__list-item">
                <RouterLink :to="{ name: 'Details', params: { slug: post.slug?.current } }">
                    <img class="projects__list-item__img"
                        :src="getSanityImageUrl(post.contentImage, { maxWidth: 700, aspectRatio: 1 })"
                        :alt="post.contentImage?.alt || post.title" :sizes
                        :srcset="getSanityImageSrcSet(post.contentImage, { maxWidth: 700, aspectRatio: 1 })"
                        width="700">
